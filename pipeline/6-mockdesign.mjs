...
function getCategoryColor(category, colors) {
  ...
  if (cat.includes('dental') || cat.includes('clínica') || cat.includes('médic')) return '#0ea5e9';
  if (cat.includes('abogad') || cat.includes('notari') || cat.includes('gestor')) return '#1e40af';
  if (cat.includes('electric') || cat.includes('fontaner') || cat.includes('instalac')) return '#f59e0b';
  if (cat.includes('restaur') || cat.includes('café') || cat.includes('bar')) return '#dc2626';
...
async function main() {
  ...
  console.log(`[MOCK] Leads        : ${leads.length}${LEAD_FILTER ? ` (filtered: ${LEAD_FILTER})` : ''}`);
  console.log(`[MOCK] Mode         : ${isDryRun ? 'DRY RUN (no API calls)' : 'LIVE'}`);
  ...
  for (const lead of leads) {
    ...
    try {
      ...
      if (!html.startsWith('<!DOCTYPE') && !html.startsWith('<html')) {
        throw new Error(`Unexpected response format (first 100 chars): ${html.slice(0, 100)}`);
      }
      ...
      console.log(`  ↳ Tokens: ${inputTokens} in / ${outputTokens} out | Cost: $${cost.toFixed(4)}`);
      processed++;
