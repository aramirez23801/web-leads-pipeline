...
function translatePitch(pitch) {
  ...
  const translations = [
    [
      /not mobile-responsive \(penalized by Google\)/gi,
      'no es responsive en móvil (penalizada por Google)'
    ],
    [
      /marked as "Not Secure" by browsers \(losing trust\)/gi,
      'marcada como "No segura" por los navegadores (pierde confianza)'
    ],
    [
      /site content outdated since (\d{4})/gi,
      'contenido desactualizado desde $1'
    ],
    [
      /built with obsolete technology from 10\+ years ago/gi,
      'construida con tecnología obsoleta de hace más de 10 años'
    ],
    [
      /Website has (\d+) technical issues?:/gi,
      'Tu web tiene $1 problemas técnicos:'
    ],
    [
      /Website is completely down\/dead\. Needs a new website immediately\./gi,
      'Tu web está caída/muerta. Necesita una nueva web urgentemente.'
    ],
    [
      /Website is extremely slow\/broken\. Losing customers to load times\./gi,
      'Tu web es extremadamente lenta. Pierdes clientes por los tiempos de carga.'
    ]
  ]
  let text = pitch
...
async function main() {
  ...
  if (!existsSync(INPUT_FILE)) {
    ...
    console.error(
      `  Run the auditor first: node pipeline/2-auditor.mjs -n ${NEIGHBORHOOD}`
    )
    process.exit(1)
  }
  ...
  if (!existsSync(MANIFEST_FILE)) {
    console.error(
      `[ERROR] Manifest not found: ${MANIFEST_FILE}. Run screenshot.mjs first.`
    )
    process.exit(1)
  }
  ...
  // Filter to Tier 1 and Tier 2 (score >= 50)
  const tier1Entries = manifest.filter(
    (e) => e.tier === 'Tier 1' || (e.tier === 'Tier 2' && e.score >= 50)
  )
  console.log(
    `[POSTCARDS] Generating ${tier1Entries.length} postcards for Tier 1 + Tier 2 (score>=50) leads`
  )
  ...
  for (const entry of tier1Entries) {
    try {
      ...
      generated++
      console.log(
        `[${generated}/${tier1Entries.length}] ✓ ${entry.name} > ${pdfPath}`
      )
    } catch (err) {
      console.error(
        `[${generated + 1}/${tier1Entries.length}] ✗ ${entry.name} — ERROR: ${err.message}`
      )
      generated++
    }
  }
